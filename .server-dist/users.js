...
function importUsergroups() {
	...
	return _fs.FS.call(void 0, 'config/usergroups.csv').readIfExists().then(data => {
...
function exportUsergroups() {
	...
	return _fs.FS.call(void 0, 'config/usergroups.csv').write(buffer);
}
...
// User
 class User extends Chat.MessageContext {
	...
	async tryJoinRoom(roomid, connection) {
		roomid = roomid && (roomid ).id ? (roomid ).id : roomid ;
	...
	for (const user of users.values()) {
		...
		const bypass = user.statusType !== 'online' ||
			(!user.can('bypassall') &&
				...
				Array.from(user.inRooms).some(room => user.can('bypassafktimer', null, Rooms(room) ))));
...
function socketConnect(worker, workerid, socketid, ip, protocol) {
	...
	if (Config.emergency) {
		// tslint:disable-next-line:no-floating-promises
		_fs.FS.call(void 0, 'logs/cons.emergency.log').append('[' + ip + ']\n');
	}
...
function socketReceive(worker, workerid, socketid, message) {
	...
	if (Config.emergency) {
		// tslint:disable-next-line:no-floating-promises
		_fs.FS.call(void 0, 'logs/emergency.log').append(`[${user} (${connection.ip})] ${roomId}|${message}\n`);
	}
