...
const baseCommands = undefined;
const commands = undefined;
const basePages = undefined;
...
const hostfilters = [];
function hostfilter(host, user, connection, hostType) {
...
const loginfilters = [];
function loginfilter(user, oldUser, usertype) {
	for (const curFilter of exports.Chat.loginfilters) {
		curFilter(user, oldUser, usertype);
	}
}

const nicknamefilters = [];
function nicknamefilter(nickname, user) {
...
const statusfilters = [];
function statusfilter(status, user) {
...
_fs.FS.call(void 0, TRANSLATION_DIRECTORY).readdir().then(files => {
...
function tr(language, strings, ...keys) {
	...
	if (!strings.length) {
		return ((fStrings, ...fKeys) => {
			exports.Chat.tr(language, fStrings, ...fKeys);
		});
	}
 ...
 */
function parse(message, room, user, connection) {
	exports.Chat.loadPlugins();
	const context = new CommandContext({message, room, user, connection});

	return context.parse();
}
function sendPM(message, user, pmTarget, onlyRecipient = null) {
...
function loadPlugins() {
	...
	// tslint:disable-next-line: no-floating-promises
	_fs.FS.call(void 0, 'package.json').readIfExists().then(data => {
 ...
 * words.
 */
function plural(num, pluralSuffix = 's', singular = '') {
 ...
 *
 */
function count(num, pluralSuffix, singular = "") {
	if (num && typeof num.length === 'number') {
		num = num.length;
	} else if (num && typeof num.size === 'number') {
		num = num.size;
 ...
 */
function toTimestamp(date, options = {}) {
 ...
 *
 */
function toDurationString(val, options = {}) {
...
function collapseLineBreaksHTML(htmlContent) {
	htmlContent = htmlContent.replace(/<[^>]*>/g, tag => tag.replace(/\n/g, ' '));
	htmlContent = htmlContent.replace(/\n/g, '&#10;');
	return html;
}

function getDataPokemonHTML(template, gen = 7, tier = '') {
...
function getDataAbilityHTML(ability) {
 ...
 */
async function fitImage(url, maxHeight = 300, maxWidth = 300) {
 ...
 */
function maybeNotifyBlocked(blocked, targetUser, user) {
 ...
 */
const forceRenames = new Map();

function registerMonitor(id, entry) {
	if (!exports.Chat.filterWords[id]) exports.Chat.filterWords[id] = [];
	exports.Chat.monitors[id] = entry;
}

function resolvePage(pageid, user, connection) {
 ...
 const Chat = {
	...
	formatText: _chatformatter.formatText,
	linkRegex: _chatformatter.linkRegex,
	stripFormatting: _chatformatter.stripFormatting,
